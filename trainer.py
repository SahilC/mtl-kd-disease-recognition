...
class KDMultiTaskTrainer(BaseTrainer):
    ...
    def distillation_loss(self, y, labels, teacher_scores, alpha = 0.95, reduction_kd='mean', task = 0):
        ...
        if self.kd_type == 'self':
            ...
            d_loss = self.criterion(y, labels)
        else:
            d_loss = torch.nn.KLDivLoss(reduction=reduction_kd)(F.log_softmax(y/ T, dim= -1), F.softmax(teacher_scores / T, dim=-1)) * T * T
        nll_loss = self.criterion(y, labels)
        if self.kd_type == 'full':
            tol_loss = alpha * d_loss + (1.0 - alpha) * nll_loss
        elif self.kd_type == 'nll_only':
            tol_loss = nll_loss
        elif self.kd_type == 'kd_only' or self.kd_type == 'self':
            tol_loss = d_loss
        else:
            raise NotImplementedError
        return tol_loss, d_loss, nll_loss

    def compute_loss(self, predictions, gt, teacher_preds): 
       disease,f_disease, text_pred = predictions
       labels, f_labels, text = gt
       t_labels, t_f_labels, t_text = teacher_preds
       t_labels = t_labels.detach()
    ...
    def train_iteration(self, train_loader):
        ...
        for i, (_, images, labels, f_labels, text) in enumerate(train_loader):
               ...
               if i != 0 and i % self.print_every == 0:
                  (train_loss, avg_text_loss, accuracy, total_disease_acc, bleu,
                          total_train_loss, total_tl1, total_tl2, total_tl3) = list(map(lambda x: x/self.print_every, [train_loss, loss3, accuracy, total_disease_acc, bleu, 
                                                                                                                        total_train_loss, total_tl1, total_tl2, total_tl3]))
                  ...
                  print('Iter:{}\tTraining Loss:{:.8f}\tAcc:{:.8f}\tDAcc:{:.8f}\tBLEU:{:.8f}\tTextLoss:{:.8f}'.format(i, train_loss, accuracy/batch_size,
                              total_disease_acc / batch_size,
                              bleu,
                              loss3.item()))

                  count, train_loss, text_loss, accuracy, total_disease_acc, bleu, total_tl1, total_tl2, total_tl3, total_train_loss = [0.0 for i in range(10)]
    ...
    def validate(self, val_loader, epoch = 0):
        ...
        with torch.no_grad():
            for i, (_, images, labels, f_labels, text) in enumerate(val_loader):
                ...
                loss, loss1, loss2, text_loss = self.compute_loss((diseases, fine_diseases, text_pred), (labels, f_labels, text), teacher_preds)
                ...
                acc, recall, precision, f1 = accuracy_recall_precision_f1(d_pred, labels)
        ...
        (val_loss, total_l1, total_l2, total_l3, total_acc, total_d_acc, 
                total_f1, total_precision, total_recall, total_cm, bleu) = list(map(lambda x: x/ len(val_loader), [val_loss, total_l1, total_l2, total_l3, total_acc, total_d_acc, 
                                                                                                                   total_f1, total_precision, total_recall, total_cm, bleu]))
